...
//        cout << "Reading params from: " << foldername << "...";
        ...
        for (int i = 0; i < p->noPatches; i++)
        {
            ...
            Mat patchFeatsAux = RV_readCSVMat(foldername + "patchFeats/patchFeats_" + to_string(i + 1) + ".csv", p->d, 8);
...
//        cout << "done!" << endl;
    ...
    void RV_featExtractionWholeFrame(Mat &frame, paramList params, Mat &featData)
    {
        // function that computes the same features as in the FG paper (9 channels and all)
        // but doesn't use overlapping patches (is extracts feats from the whole image)
...
//            (*bgPerc)[k] = countBG / (params.patchSize - 1) / (params.patchSize - 1);
    ...
    double computePainLevel(vector<double> pred){
//        std::vector<double>::iterator maxs = std::max_element(pred.begin(), pred.end()); //returns all mins
//        double max = maxs[0]; //select the zeroth min if multiple mins exist
//        int argMax= 0;
        double painUnit = 1.0/pred.size();
        double painLevel = 0.0;
        for(int i=0; i < pred.size(); i++)
        {
            painLevel += pred[i] * i * painUnit;
            //Note: could use fabs( (min - vec[i]) < 0.01) if worried about floating-point precision
//            if(pred[i] == max)
//                argMax = i;
        }
        return painLevel* 100;
    }
}
